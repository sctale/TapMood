...
### 优化
- **备份文件名精确到秒**：`tapmood_backup_YYYYMMDD.json` → `tapmood_backup_YYYYMMDD_HHmmss.json`，同日多次导出不再互相覆盖/产生 `(1)` 后缀副本；分享导出与保存到手机共用同一命名
...
### 优化
- **"保存到手机"拉起失败可自我诊断**：不再吞掉 `Linking.openURL` 的真实异常（此前统一显示"无法拉起系统写入入口"，无法区分是 resolver 拒绝还是 startActivity 抛错）。现在深链与兜底两条路径的原始错误消息都会带进弹窗文案；并新增第二通道——`intent:` URI 显式组件直连（不经隐式过滤器匹配），原生入口同时接受 query 深链与 `name` extra 两种传参
- **交付物位置约定**：release notes / 发版 APK 统一存放于项目根目录（`release_notes*.md` 加入 .gitignore），不再落入通用输出目录
...
### 修复
- **启动引导页三页挤成一屏（分页失效）**：v0.3.40 代码审查批量修复（`dd2432c`）将 `Dimensions.get` 迁移为 `useWindowDimensions` 时，删除了 `styles.page` 的固定宽度却未按注释真正注入 `{ width }`，横向 ScrollView 内三页被 `flex:1` 压缩进同一屏宽——滑动分页失效、"开始使用"CTA 与圆点跳页位置错乱。现已在页面容器上动态注入视口宽度（旋转/iPad 分屏同步响应），并同步恢复圆点跳页与页码计算的正确性

## [0.3.49] - 2026-09-13

### 安全
- **改用私有 release keystore 签名**：此前所有版本的 release APK 复用了公开的 debug keystore（`CN=Android Debug`），存在升级劫持风险。本次起改用项目私有 keystore（`CN=TapMood Release`），由新增 config plugin `plugins/withReleaseSigning.js` 在 prebuild 时注入，keystore 与密码存放于 `keystores/`（已 gitignore，不入库）。
  - ⚠️ **升级须知**：签名证书变更后，旧版（debug 签名）无法直接覆盖安装。老用户需**先卸载旧版本再安装 v0.3.49**，否则系统会提示"安装包冲突/签名不一致"。本地数据存于应用私有目录，卸载会清除，请先在设置页导出备份。

### 修复
- **导入数据后首页数据陈旧**：设置页导入（合并/替换）后切回首页，今日心情/日历/连续打卡/周期进度不刷新。HomeScreen 新增 `DATA_IMPORTED` 事件监听，导入完成即刷新
- **导入备份的通知设置零校验**：非法 `hour`/`minute`（超范围、非数字）会持久化并导致调度产生 Invalid Date，旧通知被清空、新通知调度失败，通知链跨启动断裂。导入时校验设置字段，非法值回落默认；调度改为"先算日期、后清旧调度"，杜绝先清后失
- **日历弹窗补记今天后提醒仍触发**：通过日期弹窗记录今日心情后未重新调度通知（主链路有此逻辑），当天已排定的提醒仍会弹出。弹窗记录成功后补上通知重调度
- **设置页"数据库大小"永不显示**：`getDatabaseSize` 用文档目录根拼路径，而 expo-sqlite 实际存放在 `defaultDatabaseDirectory`（多一层 `SQLite/` 子目录），文件恒不存在返回 0。改用正确目录拼路径
- **iOS 小组件跨日 streak 陈旧**：时间线明日条目 `todayMood` 清空但 `streak` 沿用今日值，与"今天未记录即 0"的语义矛盾。明日条目 `streak` 置 0
- **进程常驻跨天后日期范围固化**：首页/分析页的"本周/本月/本年"范围仅在切换周期时计算，App 常驻跨周/月后仍指向旧区间。回前台检测到跨天时重算范围（首页未手动翻页则自动跟进新今天）
- **Toast 同文案连续触发提前消失**：`React.memo` 下相同文案的连续提示因 props 浅相等不重渲染、定时器不重置。Toast 新增自增 `id` 令牌，重复文案也能重置倒计时
- **通知开关保存失败时 UI 与 DB 不一致**：开关先更新 UI 再持久化，保存抛错时 Switch 显示开启而库中仍为关闭。改为先持久化、成功再更新 UI

## [0.3.48] - 2026-09-06

### 变更
- **心情按钮排序全局改为 好→中→差**：由原"差/中/好"调整为"好/中/差"（正向优先），App 内 MoodSelector、日期编辑弹窗、柱状图/饼图图例与 Android/iOS 桌面小组件三按钮全部同步；MoodTrendChart 纵轴本就按心情值（好在上）排布，无需改动
- **引导页文案同步**：第 2 屏副标"差 / 中 / 好"改为"好 / 中 / 差"

### 优化
- **心情顺序收敛为单一数据源**：iOS 小组件按钮/校验、导入数据心情校验（importData）、冷启动小组件待处理心情校验（HomeScreen）不再各自硬编码 `['bad','okay','good']`，统一消费 `MOOD_LEVELS`，杜绝未来改序漏改
- **心情映射键序统一为 好/中/差**：`MOOD_CONFIG`、MoodIcon 描边/背景表、MoodSelector 选中文字色表、TodayStatus 分享 emoji 表按新顺序重排（仅代码可读性，行为不变）

## [0.3.47] - 2026-08-15

### 修复
- **通知权限请求时机调整**：移除首次启动引导页的提前权限请求，改为用户主动开启"每日提醒"开关时才请求系统通知权限（符合"功能相关权限在使用时才请求"的最佳实践）；此前引导页先请求，导致用户已授权后开启定时不弹权限提醒

## [0.3.46] - 2026-08-15

### 变更
- **小组件心情按钮重绘为实心彩色圆**：三个按钮改为靛蓝/琥珀/薄荷绿主色实色圆底 + 加粗表情（笔画按底色亮度适配白/深棕/深绿，小尺寸下清晰可辨）；去掉原"浅色圆角矩形套淡色圆"双层结构（边界模糊显脏）与过细笔画（1.8px 在桌面尺寸下几乎不可见）

## [0.3.45] - 2026-08-15

### 变更
- **Android 小组件恢复纯三按钮极简形态**：移除左侧状态区文字（🔥连续天数/✓/···），仅显示 3 个心情按钮；保留已记录按钮全亮、其余半透明的透明度反馈

### 优化
- **小组件刷新省去 streak 查询**：widget_state.json 不再写入连续打卡字段（原生侧已不读取），每次刷新减少一次数据库查询
